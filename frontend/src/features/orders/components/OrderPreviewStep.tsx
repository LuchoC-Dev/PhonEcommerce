...
export function OrderPreviewStep({ preview, onConfirm, onBack, isLoading, error }: Props) {
  return (
    <div className="flex flex-col gap-6">
        ...
        {preview.items.map((item) => (
          ...
          >
            <div className="flex flex-col gap-0.5 min-w-0">
              <p className="text-sm font-medium text-[--color-text] truncate font-[--font-display]">
                {item.productName}
              </p>
      ...
      <div className="p-4 rounded-[--radius-lg] bg-[--color-surface] border border-[--color-border]">
        <p className="text-xs font-medium text-[--color-text-muted] mb-2 uppercase tracking-wide">Envío a</p>
        <p className="text-sm text-[--color-text]">{preview.shipping.name}</p>
        ...
        {preview.shipping.phone && (
          <p className="text-sm text-[--color-text-muted]">{preview.shipping.phone}</p>
        )}
      </div>
